...
function extractNeeds(text) {
  ...
  // offer
  if (/(\d+)\s*折/.test(t)) { const m = t.match(/(\d+)\s*折/); out.offer = m[1] + '折优惠'; }
...
class IGDE {
  ...
  /** 无条件落库：把抽取到的意图合并进 act.needs（已明确字段绝不再覆盖为用户否认之外） */
  applyNeeds(act, extracted) {
    act.needs = act.needs || {};
    for (const f of NEEDED_FIELDS) {
      if (extracted && extracted[f] != null && String(extracted[f]).trim()) {
        act.needs[f] = extracted[f];
      }
    }
    return act.needs;
  }
  ...
  async handle(act, userText, opts = {}) {
    ...
    if (this.aiEnabled && this.callAI) {
      try {
        const r = await this._aiCoach(act, userText); // 一次对话同时抽取 needs + 生成话术
    ...
    if (l2ok && this.callCritic) {
      try { l2ok = await this.callCritic(reply); } catch (e) { l2ok = false; } // fail-closed
    }
    if (!l2ok) {
      const regen = await this._tryRegen(act, userText, 'preachy');
      if (regen && (await this._passL2(regen))) { reply = regen; guardrailHits.push('L2regen'); }
      else { reply = this._pickFallback(act); guardrailHits.push('L2'); }
    }
    // L4 抢跑禁令（S3 前不得出方案卡式配置）
    if (!guardrailL4(reply, act.stage)) {
      const regen = await this._tryRegen(act, userText, 'preempt');
    ...
    return { reply, stage: act.stage, needs: act.needs, planCard, guardrailHits };
  }
  ...
  /** 真实模型：一次对话同时完成「意图抽取 + 话术生成」
   *  用 lib/llm 的 buildCoachMessages 组装（人格系统提示词 + 完整多轮历史 + 当前句），
   *  适配器 callAI 返回 {reply, needs} 或 JSON 字符串（chatStructured 一次出两个字段）。 */
  async _aiCoach(act, userText) {
    const messages = buildCoachMessages({ act, userText, needs: act.needs, stage: act.stage, missing: this.missingFields(act) });
    const res = await this.callAI(messages);
    let reply = '', needs = {};
    if (typeof res === 'string') {
      try { const j = JSON.parse(res); reply = j.reply || ''; needs = j.needs || {}; }
   ...
   *  复用 buildCoachMessages 组装上下文，并把约束追加进末句，避免输出违规/抢跑内容。 */
  async _tryRegen(act, userText, why) {
    ...
    const messages = buildCoachMessages({ act, userText: regenText, needs: act.needs, stage: act.stage, missing: this.missingFields(act) });
    try {
      const res = await this.callAI(messages);
      let r = '';
      if (typeof res === 'string') {
